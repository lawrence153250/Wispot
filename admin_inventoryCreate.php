...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_equipment'])) {
    ...
    // Insert new inventory item with 'available' status immediately
    $stmt = $conn->prepare("INSERT INTO inventory 
                          (itemName, itemType, quantity, description, status, dateAdded) 
                          VALUES (?, ?, ?, ?, 'available', NOW())");
    $stmt->bind_param("ssis", $itemName, $itemType, $quantity, $description);
    
    if ($stmt->execute()) {
        $success_message = "Equipment added successfully and made available immediately!";
    } else {
        ...
        <div class="inventory-form-container">
            <div class="inventory-form-card">
                <div class="form-card-header">
                    <h4><i class="bi bi-box-seam"></i> Add New Equipment (Admin)</h4>
                    <p>Create equipment that will be immediately available in inventory</p>
                </div>
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <button type="submit" name="add_equipment" class="btn-add-equipment">
                        <i class="bi bi-check-circle"></i> Add Equipment (Immediate Availability)
                    </button>
                </form>
            </div>
        </div>
        ...
        <div class="equipment-cards-section">
            ...
            <?php if (empty($inventoryItems)): ?>
                <div class="alert alert-info">
                    <i class="bi bi-info-circle"></i> No inventory items found. Add your first equipment above.
                </div>
            <?php else: ?>
                <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="equipment-card">
                                ...
                                <div class="equipment-dates">
                                    ...
                                    <?php echo ($item['status'] === 'available') ? 'Available' : 'Submitted'; ?>: <?php echo date('M d, Y', strtotime($item['dateAdded'])); ?>
                                </div>
                                
                                <div class="equipment-meta">
                                    <span class="submitted-date">
                                        <i class="bi bi-clock"></i>
                                        <?php echo ($item['status'] === 'available') ? 'Added' : 'Submitted'; ?>: <?php echo date('M d, Y H:i', strtotime($item['dateAdded'])); ?>
                                    </span>
                                    <?php if (!empty($item['staffUsername'])): ?>
                                        <span class="submitted-by">
                                            <i class="bi bi-person"></i>
                                            By: <?php echo htmlspecialchars($item['staffUsername']); ?>
                                        </span>
                                    <?php endif; ?>
                                </div>
                            </div>
