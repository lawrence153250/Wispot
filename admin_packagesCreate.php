...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_package'])) {
    ...
    if (empty($selectedItems)) {
        ...
        // Insert new package with all fields - status is 'available' immediately for admin
        $stmt = $conn->prepare("INSERT INTO package (
            packageName, description, price, equipmentsIncluded, 
        ...
        if ($stmt->execute()) {
            $success_message = "Package created successfully and made available immediately!";
        } else {
...
// Fetch ALL packages with proper date fields
$packages = [];
...
<head>
    ...
    <title>Admin Packages Creation</title>
        ...
        <div class="package-form-container">
            <div class="package-form-card">
                <div class="form-card-header">
                    <h4><i class="bi bi-box-seam"></i> Create New Package (Admin)</h4>
                    <p>Design a new package that will be immediately available</p>
                </div>
                ...
                <form method="POST">
                    ...
                    <button type="submit" name="create_package" class="btn-create-package">
                        <i class="bi bi-check-circle"></i> Create Package (Immediate Availability)
                    </button>
                </form>
            </div>
        </div>
